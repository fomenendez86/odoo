...
def generate_password():
    ...
    try:
        ...
        safe_command.run(subprocess.run, ('sudo', 'usermod', '-p', shadow_password, 'pi'), check=True)
        with writable():
            safe_command.run(subprocess.run, ('sudo', 'cp', '/etc/shadow', '/root_bypass_ramdisks/etc/shadow'), check=True)
        return password
